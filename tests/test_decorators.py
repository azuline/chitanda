...
async def test_admin_only_decorator_generator():
    ...
    assert [1, 2, 3] == [
        n async for n in admin_only(func)(Message(None, listener, 2, "azul", 4, 5))
    ]
...
async def test_admin_only_decorator_not_generator():
    ...
    assert "hello" == await admin_only(func)(Message(None, listener, 2, "azul", 4, 5))


@pytest.mark.asyncio
async def test_admin_only_decorator_error():
    listener = Mock(is_admin=AsyncMock(return_value=False))
    with pytest.raises(BotError):
...
async def test_auth_only_decorator_generator():
    ...
    listener = Mock(is_authed=AsyncMock(return_value=True))
    assert [1, 2, 3] == [
        n async for n in auth_only(func)(Message(None, listener, 2, "azul", 4, 5))
    ]
...
async def test_auth_only_decorator_not_generator():
    listener = Mock(is_authed=AsyncMock(return_value=True))
    func = AsyncMock(return_value="hello")
    assert "hello" == await auth_only(func)(Message(None, listener, 2, "azul", 4, 5))


@pytest.mark.asyncio
async def test_auth_only_decorator_error():
    listener = Mock(is_authed=AsyncMock(return_value=False))
    with pytest.raises(BotError):
...
def test_private_message_only_decorator():
    func = Mock(return_value="hello")
    assert "hello" == private_message_only(func)(Message(None, 1, 2, 3, 4, True))
...
def test_allowed_listeners_decorator_one():
    assert "hello" == allowed_listeners(IRCListener)(Mock(return_value="hello"))(
